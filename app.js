...
// Send a TwiML response with the given response object to play the given
...
module.exports = function appctor(cfg) {
  ...
  app.post('/',function(req,res,next) {
    ...
    bcrypt.hash(req.body.passcode, 10, function(err, passhash) { 
      ...
      // Add all the standard config variables to the HMSET operation
    ...
    db.multi()
      ...
      .exec(function (err, dbres) {
        ...
        if (dbres[1]) {
          ...
          db.del(cid + ' unlocked');
          // We run this command without a callback. If the deletion failed,
          // there will probably be bigger issues than the fact that we let
          // someone in without waiting for confirmation that the door
          // was re-locked- either way, we want to let them in, right?
          
          // Play the tone sequence that unlocks the door
